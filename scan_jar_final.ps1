...
function CheckEntry($entryName, $names) {
    ...
    foreach ($n in $names) {
        $found = $s.Contains($n)
        if ($found) { $status = 'FOUND' } else { $status = 'MISSING' }
        Write-Output ($entryName + ' -> ' + $n + ' : ' + $status)
    }
}

$particleNames = @('SMOKE_NORMAL','SMOKE_LARGE','SPELL_WITCH','VILLAGER_HAPPY','PORTAL','SNOWFLAKE','CRIT_MAGIC','EXPLOSION_LARGE','BLOCK_CRACK','BLOCK_DUST','ELECTRIC_SPARK','FLAME','CLOUD','EXPLOSION_HUGE','SNOWBALL')
...
CheckEntry 'org/bukkit/potion/PotionEffectType.class' $potionNames
