<div class="spell-page-shell">
	<section class="spell-hero">
		<p class="spell-eyebrow">Witch Plugin</p>
		<h1>Minecraft Cod SMP Full Spell Ability List</h1>
		<p class="spell-subtitle">A dark, markdown-driven spell compendium organized by school, with a live outline on the side.</p>
		<div class="spell-loader-state" id="spell-loader-state">Loading spell markdown...</div>
	</section>

	<section class="spell-layout">
		<aside class="spell-outline-panel">
			<div class="spell-outline-header">
				<p class="spell-outline-eyebrow">On This Page</p>
				<h2>Spell Schools</h2>
			</div>
			<nav id="spell-outline" class="spell-outline" aria-label="Spell section outline"></nav>
		</aside>

		<section class="spell-panel">
			<article id="spell-markdown" class="spell-markdown" aria-live="polite"></article>
		</section>
	</section>
</div>

<script>
	window.RELICBOUND_SPELL_MARKDOWN_URL = 'https://raw.githubusercontent.com/codsmp/Plugin/main/docs/spell-compendium.md';
</script>
